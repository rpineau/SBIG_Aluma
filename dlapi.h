...
namespace dl
{
	...
	enum EStatusCode
	{
		...
		UnsupportedBodyType,		//!< Invalid/unsupported body type supplied in request header

		// device specific errors ... 
		DeviceError = 0x80,					//!< Start of device-side error codes
		DeviceBusy,							//!< Catch-all for device busy errors
		ExposureInProgress = DeviceBusy,	//!< Device has an exposure in progress and cannot comply [[deprecated]]
		ReadoutInProgress = DeviceBusy, 	//!< Device has a frame readout in progress and cannot comply [[deprecated]]
		ImageNotAvailable,					//!< Image Transfer was requested, but no image is ready for download			
		FeatureNotSupported,				//!< Camera does not support the requested feature. \see ICamera::Capability, ICamera::getCapability(), ICamera::queryCapability()
	...
	*/
	...
	enum EFrameType
	{
		...
		RollingShutter,				//!< Rolling Shutter for CMOS sensors.
		GlobalShutter,				//!< Global Shutter for CMOS sensors.
	...
	*/
	 ...
	 */
#	pragma pack(push, DL_STRUCTALIGN)
	...
	*/
	struct TExposureOptions
	{
		...
		char binX;					//!< [[deprecated]] Binning of the exposure in the x-direction in pixels. Is unused, and will go away in future versions of DLAPI.
		char binY;					//!< [[deprecated]] Binning of the exposure in the y-direction in pixels. Is unused, and will go away in future versions of DLAPI.
...
#	pragma pack(pop)
	...
	public:
		...
		/*!
		 *	\brief Returns the API status code reported by the execution of the command
		 *  \return API status code reported by the camera
		 *	\see EStatusCode
		 *
		 *  This function returns a more fine-grained error code from the API when a failure is encountered.
		 *	Some error codes are recoverable (such as the feature not supported), and we need to 
		 *
		 */
	...
	*/
	...
	public:
		...
		enum Model
		{
			...
			Internal,		//!< STC Internal FW

			// New AFW line where model number represents number of slots (submodels include filter diameter & shape)
			AFW16,	//!< AFW-16 slot filter wheel
			AFW15,	//!< AFW-15 slot filter wheel
			AFW14,	//!< AFW-14 slot filter wheel
			AFW13,	//!< AFW-13 slot filter wheel
			AFW12,	//!< AFW-12 slot filter wheel
			AFW11,	//!< AFW-11 slot filter wheel
			AFW10,	//!< AFW-10 slot filter wheel
			AFW9,	//!< AFW-09 slot filter wheel
			AFW8,	//!< AFW-08 slot filter wheel
			AFW7,	//!< AFW-07 slot filter wheel
			AFW6,	//!< AFW-06 slot filter wheel
			AFW5,	//!< AFW-05 slot filter wheel
			AFW4,	//!< AFW-04 slot filter wheel
			AFW3,	//!< AFW-03 slot filter wheel
			AFW2,	//!< AFW-02 slot filter wheel
	...
	public:
		 ...
		 */
		enum Model {
			RESERVED1 = 0x00,	//!< Reserved for future use
			RESERVED2 = 0x01,	//!< Reserved for future use
			ICX694 = 0x02,		//!< Sony ICX694
			ICX814 = 0x03,		//!< Sony ICX814
			CCD4710 = 0x04,		//!< CCD 47-10
			CCD7700 = 0x05,		//!< CCD 77-00
			RESERVED3 = 0x06,	//!< Reserved for future use
			RESERVED4 = 0x07,	//!< Reserved for future use
			RESERVED5 = 0x09,	//!< Reserved for future use
			RESERVED6 = 0x08,	//!< Reserved for future use
			KAF3200 = 0x0a,		//!< Kodak KAF 3200
			KAF8300 = 0x0b,		//!< Kodak KAF 8300

			RESERVED7 = 0x10,	//!< Reserved for future use
			RESERVED8 = 0x11,	//!< Reserved for future use

			RESERVEDB1 = 0x100,	//!< Reserved for subvariant 1
			
			RESERVEDC1 = 0x200,	//!< Reserved for subvariant 3

			GSENSE4040 = 0x300,	//!< GPixel GSENSE4040
			GSENSE2020 = 0x600,	//!< GPixel GSENSE2020 

			P1300 = 0x400,		//!< Python 1300

			IMX428 = 0x500,		//!< Sony ICX428
			IMX428_7,			//!< Sony ICX428 (STC-7 Variant)
...
#		pragma pack(push, DL_STRUCTALIGN)
		 ...
		 *  \see ISensor::Setting, ISensor::querySetting(), ISensor::getSetting(), and ISensor::setSetting()
		 *
		 *  Structure is reflected by the ISensor::Setting index keys, and contains all user-controlled settings.
		 *  NOTE: This strucure is deprecated. Please use the ISensor::querySetting(), ISensor::getSetting(), and ISensor::setSetting() methods instead.
		 *
		 */
		struct Settings
		{
			...
			int  shutterPolarity;		//!< controls the polarity of solenoid shutters. Only needs to be changed if shutter operation is inverted (i.e. requesting shutter open results in shutter closed, and vice versa)
			bool enableStatusLED;		//!< enables/disables the status LED on the side of the camera. non-zero enables it, zero disables it. defaults to on.
			int  partialExposureLength; //!< controls the length of sub-exposures during internal stacking exposures (in 1 second increments)
			int	 useFWAsShutter;		//!< true to use the connected filter wheel as a shutter (use the index of the slot, starting from 1), 0 disables this feature
		};
#		pragma pack(pop)
		...
		/*!
		 * \brief Returns the requested setting value
		 * \param ISensor::Setting key the key value to retrieve
		 * \return returns the setting value specified with key
		 * \see ISensor::Settings, ISensor::Setting, ISensor::querySettings()
		 * 
		...
		/*!
		 *  \brief Start an exposure using this sensor with aditional information.
		 *	\id id of the exposure taken
		 *	\exposureType exposure information ("Locate_Image", "Cal_Image", "Track_Image")
		 *  \param value the parameters of the exposure.
		 *  \return IPromisePtr pointer to promise for monitoring the status of the asynchronous operation.
		 *  \see startExposure
		 */
		...
		*/
		...
		enum AdjustableGainMode {
			...
			eAdjustableADCGainFirst = eAdjustableADCGain0,

			eAdjustableGainAC4040HiGain = eAdjustableADCGain0,
			eAdjustableGainAC4040LoGain = eAdjustableADCGain1
		};

		/*!
		 * \brief getAdjustableGains()
		 * \param mode the index of the ADC converter you wish to fetch gain modes for. See full description for mappings.
		 * \param pGainEntries a pointer to an array of integers that will receive the complete list of gain entries.
		 * \param gainEntryCount [IN] the size of the array passed into pGainEntries. [OUT] Number of entries put into pGainEntries.
		 * \see ISensor::setSetting(), ISensor::Setting::AnalogGain0, ISensor::Setting::AnalogGain1, ICamera::Capability::eSupportsAdjustableGain, ISensor::getAdjustableGainCount()
		 * 
		 * Returns the allowed analog gain values user can pass into ISensor::Setting::AnalogGain0 and ISensor::Setting::AnalogGain1. These
		 * gains refer to adjustable analog converter gains. Measured EGain values are scaled in the camera to match the new analog gains.
		 * We recommend passing in a buffer with a minimum size equal to the value returned by ISensor::getAdjustableGainCount().
		 * 
		 * e.g.
		 * ```c++
		...
		/*!
		 * \brief getAdjustableGainCount()
		 * \param mode the index of the ADC converter you wish to fetch gain mode count for
		 * \returns the maximum number of allowed analog gain options returned by ISensor::getAdjustableGains()
		 * \see ISensor::setSetting(), ISensor::Setting::AnalogGain0, ISensor::Setting::AnalogGain1, ICamera::Capability::eSupportsAdjustableGain
		 * 
		 */
		virtual unsigned int getAdjustableGainCount(AdjustableGainMode mode) const = 0;

		/*!
		 * \brief getDefaultAdjustableGain()
		 * \param mode the index of the ADC converter you wish to fetch the default gain value for
		 * \returns the default gain value of the specified Adjustable Gain mode
		 */
	...
	public:	
		 ...
		 */
		enum Model {
			Aluma,			//!< Aluma and AlumaCCD model cameras
			Reserved1,		//!< Reserved for future use
			Reserved2,		//!< Reserved for future use
			AlumaCMOS,		//!< Aluma AC4040 and AC4040BSI
			StarChaser,		//!< StarChaser model cameras
			STC,			//!< STC-7 and STC-428
			AlumaAC2020,	//!< Aluma AC2020 only
			Reserved7,		//!< Reserved for future use

			InvalidCameraModel = 0xff,	//!< Camera has not been initialized/reported invalid model
			Reserved4 = StarChaser,		//!< Historical Convenience
			Reserved5 = STC,
			Reserved3 = AlumaCMOS,
		...
		/*!
		 * \enum ICamera::Capability
		 * \brief list of camera capabilities as supported by the camera
		 */
		enum Capability {
			...
			InvalidCameraCapability = 0xff	//!< Capability has not been initialized/reported an invalid/unsupported feature
		};
...
#	pragma pack(push, DL_STRUCTALIGN)
		struct Status
		{
			...
			unsigned int    pulseGuideStatus;       //!< Status of the pulse guide relays. /seealso PulseGuideStatus
...
#		pragma pack(pop)
		...
		/*!
		 * \brief Queries a camera for a specific capability
		 * \param key the capability you wish to retrieve from the camera.
		 * \return IPromisePtr pointer to promise for monitoring the status of the asynchronous operation.
		 * \see IPromisePtr, ICamera::Capability, ICamera::getCapability()
		 *
		 * Queries the camera for the ICamera::Capability key passed in. Currently, they are used to denote support for specific features (like filter wheels and adaptive optics peripherals), but also
		 * for support to various parts of the camera (heat-sink temperature monitoring, or the use of a fan). They can be retrieved via a call to ICamera::getCapability().
		 *
		 * e.g.
		 *
		 * ```c++
		...
		/*!
		 * \brief Returns the buffered value of a successful call to ICamera::queryCapability()
		 * \param key the ICamera::Capability key to retrieve. Each key requires a separate call to ICamera::queryCapability();
		 * \return bool true if the camera feature is supported, false otherwise.
		 */
	...
	namespace V1
	{
		typedef dl::ICamera ICamera;
		typedef dl::ICamera* ICameraPtr;
	}
	...
	namespace V2
	{
		...
		typedef IConnectionManager* IConnectionManagerPtr;
	}
	...
	/*!
	 * \brief An abstract interface that gives users access to debug logging controls
	 * \seealso IGateway, getGateway()
	 * If Debug logging is supported, the IGateway pointer returned from getGateway()
	 * will inheritt from IDebugControl as well. You can check to see whether the version
	 * of DLAPI being used by your application supports these features using a dynamic
	 * cast:
	 *
	 * ```c++
	...
	public:
		...
		/*!
		 * \brief Sets debug setting value
		 * \param setting The setting to change
		 * \param val the value to write for the setting provided in parameter 1
		 * \seealso getDebugSetting(), hasDebugSetting
		 */
		virtual void setDebugSetting(IDebugControl::Setting setting, int val) = 0;

		/*!
		 * \brief Gets a debug setting value
		 * \param setting The setting to retrieve
		 * \return the value of the setting passed in. 0 if the setting is not supported.
		 * \seealso setDebugSetting(), hasDebugSetting
		 * When retrieving values from the IDebugControl instance, you should always check
		 * to see if the setting is supported:
		 *
		 * ```c++
		...
		/*!
		 * \brief Check if the IDebugControl instance supports a debug setting
		 * \param setting The setting to check
		 * \return true if supported, false otherwise.
		 * \seealso getDebugSetting(), setDebugSetting
		 * Check to see if the setting is supported by calling this function as follows:
		 *
		 * ```c++
		 * if (pDebugCtrl->hasDebugSetting(setting)
		 * {
